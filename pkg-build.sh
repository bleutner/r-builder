...
CRAN=${CRAN:-"http://cran.r-project.org"}
...
InstallDeps() {
    EnsureDevtools
    Rscript -e 'library(devtools); library(methods); options(repos=c(CRAN="'"${CRAN}"'"));  update(dev_package_deps(dependencies = c("Depends", "Imports", "Suggests", "LinkingTo")))'
}
